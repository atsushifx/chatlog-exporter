...
| `yaml`          | root が object であり、契約が要求する必須キーがすべて存在し、各値が許容型である     |
| `line-prefixed` | root が object であり、契約が要求する必須キーがすべて存在し、各値が文字列である     |
| enum を含む場合 | 当該フィールドの値が enum の許容値、またはフォールバック値のいずれかである（R-003） |
...
| `yaml` 契約のキー集合      | 呼び出し元の `extractYaml` が要求するキーと完全一致させる。過不足を作らない                |
| `line-prefixed` のキー集合 | 呼び出し元が行頭前方一致で探すキーと完全一致させる                                         |
...
| REQ-F-018      | R-001, R-007, R-008, §4.3                        | 呼び出し単位の出力契約（3 種）、契約ごとの最小構造検証、既存パーサが解釈する文字列表現への復元                                                                |
